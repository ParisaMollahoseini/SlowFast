...
class TemporalVisionTransformer(nn.Module):
    ...
    # changed by parisa for prompting
    def forward(self, x: torch.Tensor, prompt= None):
        x = self.conv1(x)  # shape = [*, width, grid, grid] 8*768*14*14
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2] 8*768*196
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        # torch.Size([128, 196, 768])

        if prompt is not None:
            prompt_repeated = prompt.unsqueeze(1).expand(-1, self.T, -1, -1)  # [5, 8, prompt_len, 768]
            a,b,c,d = prompt_repeated.shape
            prompt_repeated = prompt_repeated.reshape(a*b, c, d) # [40, prompt_len, 768]
            
            cls_token = self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device)
            x = torch.cat([cls_token, x], dim=1)  # [40, 1 + 196, 768]

            x = torch.cat([prompt_repeated, x], dim=1)  # [40, 1 + prompt_len + 196, 768]
        else:
            x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        
        real_token_count = self.positional_embedding.shape[0]  # usually 197
        x[:, -real_token_count:, :] += self.positional_embedding.to(x.dtype)
