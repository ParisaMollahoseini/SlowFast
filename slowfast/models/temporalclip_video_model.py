...
class TemporalClipVideo(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        ################### text
        #  changed by parisa. added prompt_label for adding the capability of using only label.
        if cfg.TEST.PROMPT_LABEL:
            ...
            self.text_dict = {}
            self.text_dict[0] = torch.cat([clip.tokenize(labels_dict[key]) for key in labels_dict])
            print(self.text_dict[0].shape)
            self.cls_num = self.text_dict[0].shape[0]
            print(self.cls_num)
        ################### text
        self.tune_head = cfg.TUNE_HEAD # false
        self.text_prompting = cfg.MODEL.TEXT_PROMPT # true
        self.context_length = cfg.MODEL.CONTEXT_LENGTH
        self.record_routing = cfg.MODEL.RECORD_ROUTING # false
        self.keep_raw_model = cfg.MODEL.KEEP_RAW_MODEL# false
        self.ensemble_pred = cfg.MODEL.ENSEMBLE_PRED# false
        self.distillation = cfg.MODEL.RAW_MODEL_DISTILLATION# false
        ...
        if self.tune_head:
            ...
            # added by parisa to freeze all parameters
            ...
            base_prompt  = 'a video of ' # 3 tokens
            tokenized = clip.tokenize([base_prompt]).cuda()
            with torch.no_grad():
                embedding = self.model.token_embedding(tokenized) 

            
            prompt_embed_init = embedding[:, 1:1 + self.prompt_num, :].expand(cfg.MODEL.NUM_CLASSES, -1, -1).clone()
            prompt_embed_init /= prompt_embed_init.norm(dim=-1, keepdim=True)

            requires_grad_prompt = not cfg.TEST.ENABLE
            self.prompt_embed = torch.nn.Parameter(
                prompt_embed_init,
                requires_grad=requires_grad_prompt
            )            
            ...
            self.dynamic_classifier = self.achieve_csf_matrix(self.text_dict, self.model)
            print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
            print(len(self.dynamic_classifier))
            print(self.dynamic_classifier.shape)
        ################### text
    ...
    def forward(self, x=None, update=False):
        ...
        if self.tune_head:
            ...
            # encode head.
            # print("-------------> prompt embed in forward of temporalclipvideo ")
            # print(self.prompt_embed)
            # text_embedding = torch.cat((self.token_prefix, # torch.Size([51, 1, 512])
            #             self.prompt_embed.unsqueeze(0).expand(len(self.classnames), -1, -1), 
            #             self.token_suffix # torch.Size([51, 75, 512])
            #             ), 1)
            text_embedding = torch.cat((self.token_prefix, # torch.Size([51, 1, 512])
                        self.prompt_embed, 
                        self.token_suffix # torch.Size([51, 75, 512])
                        ), 1)
            # torch.Size([51, 77]) size of tokenized_prompts
            # torch.Size([51, 77, 512]) size of text_embedding
    ...
    def achieve_csf_matrix(self, text_dict, model):
        ...
        # You are computing the element-wise mean across 28 tensors, 
        # each of shape [101, 512]. 
        # The result is a single tensor of shape [101, 512], 
        # representing the average across the 28 samples.
        
        return csf_matrix # 101 * 512
...
class WCLIP(CLIP):
    def __init__(self,
        ...
        self.visual = TemporalVisionTransformer(
            ...
            patch_size=vision_patch_size, # 16 kernel size
            width=vision_width, # 768
            layers=vision_layers, # 12
            heads=vision_heads, # 12
            output_dim=embed_dim, # 512
            T=T, # 8
            temporal_modeling_type=temporal_modeling_type, # 'expand_temporal_view'
            use_checkpoint=use_checkpoint, # false
            num_experts=num_experts, # 0
            expert_insert_layers=expert_insert_layers, # [10, 11]
            record_routing = record_routing,
            routing_type = routing_type, # 'expand_temporal_view'
        )
    ...
    def encode_image(self, image, prompt=None): # # Ensures image tensor matches model dtype parisa)
    ...
    def prompt_encode_text(self, prompts, tokenized_prompts,):
        ...
        # you're multiplying by self.text_projection, 
        # which is probably shape [hidden_dim, embed_dim], giving:
